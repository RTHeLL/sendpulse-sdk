...
class PySendPulse:
    ...
    def __send_request(self, path, method="GET", params=None, use_token=True, use_json_content_type=False):
        ...
        if response.status_code == 401 and self.__refresh_token == 0:
            ...
            logger.critical(
                "Whoops, looks like something went wrong on the server. Please contact with out support tech@sendpulse.com."
            )
        else:
            try:
                logger.debug(
    ...
    def __handle_result(self, data):
        ...
        if data.ok:
            ...
            errors = {"is_error": True, "http_code": data.status_code}
            if data.status_code == 404:
                errors["message"] = "Sorry, the page you are looking for {} could not be found.".format(
                    data.url,
                )
    ...
    def edit_addressbook(self, id, new_addressbook_name):
        ...
        """
        logger.info(
            "Function call: edit_addressbook: '{}' with new addressbook name '{}'".format(id, new_addressbook_name)
        )
        if not id or not new_addressbook_name:
            return self.__handle_error("Empty new name or addressbook id")
        return self.__handle_result(
            self.__send_request("addressbooks/{}".format(id), "PUT", {"name": new_addressbook_name})
        )
    ...
    def delete_addressbook(self, id):
        ...
        return (
            ...
            if not id
            else self.__handle_result(self.__send_request("addressbooks/{}".format(id), "DELETE"))
        )
    ...
    def get_list_of_addressbooks(self, limit=0, offset=0):
        ...
        return self.__handle_result(
            self.__send_request("addressbooks", "GET", {"limit": limit or 0, "offset": offset or 0})
        )
    ...
    def get_addressbook_variables(self, id):
        ...
        return (
            self.__handle_error("Empty addressbook id")
            if not id
            else self.__handle_result(self.__send_request("addressbooks/{}/variables".format(id)))
        )
    ...
    def get_emails_from_addressbook(self, id, limit=0, offset=0):
        ...
        return (
            ...
            if not id
                ...
                self.__send_request(
                    "addressbooks/{}/emails".format(id), "GET", {"limit": limit or 0, "offset": offset or 0}
                )
            )
        )
    ...
    def add_emails_to_addressbook(self, id, emails):
        ...
        return self.__handle_result(
            self.__send_request("addressbooks/{}/emails".format(id), "POST", {"emails": emails})
        )
    ...
    def delete_emails_from_addressbook(self, id, emails):
        ...
        try:
            ...
            return self.__handle_error("Emails list can't be converted by JSON library")
        return self.__handle_result(
            self.__send_request("addressbooks/{}/emails".format(id), "DELETE", {"emails": emails})
        )
    ...
    def get_campaign_cost(self, id):
        ...
        return (
            self.__handle_error("Empty addressbook id")
            if not id
            else self.__handle_result(self.__send_request("addressbooks/{}/cost".format(id)))
        )
    ...
    def get_list_of_campaigns(self, limit=0, offset=0):
        ...
        return self.__handle_result(
            self.__send_request("campaigns", "GET", {"limit": limit or 0, "offset": offset or 0})
        )
    ...
    def add_sender(self, email, name):
        ...
        if not name or not email:
            return self.__handle_error(
                "Seems you passing not all data for sender: Email: '{}' or Name: '{}'".format(email, name)
            )
    ...
    def delete_sender(self, email):
        ...
        """
        logger.info(
            "Function call: delete_sender: '{}'".format(
                email,
            )
        )
        return (
            self.__handle_error("Empty sender email")
            if not email
            else self.__handle_result(self.__send_request("senders", "DELETE", {"email": email}))
        )
    ...
    def send_sender_activation_email(self, email):
        ...
        """
        logger.info(
            "Function call: send_sender_activation_email for '{}'".format(
                email,
            )
        )
        return (
    ...
    def get_email_info_from_one_addressbooks(self, id, email):
        ...
        """
        logger.info(
            "Function call: get_email_info_from_one_addressbooks from: '{}'".format(
                id,
            )
        )
    ...
    def get_email_info_from_all_addressbooks(self, email):
        ...
        """
        logger.info(
            "Function call: get_email_info_from_all_addressbooks for '{}'".format(
                email,
            )
        )
        return (
            self.__handle_error("Empty email")
            if not email
            else self.__handle_result(
                self.__send_request(
                    "emails/{}".format(
    ...
    def delete_email_from_all_addressooks(self, email):
        ...
        """
        logger.info(
            "Function call: delete_email_from_all_addressooks for '{}'".format(
                email,
            )
        )
        return (
    ...
    def get_email_statistic_by_campaigns(self, email):
        ...
        """
        logger.info(
            "Function call: get_email_statistic_by_campaigns for '{}'".format(
                email,
            )
        )
        return (
    ...
    def get_emails_in_blacklist(self, limit=0, offset=0):
        ...
        return self.__handle_result(
            self.__send_request("blacklist", "GET", {"limit": limit or 0, "offset": offset or 0})
        )
    ...
    def add_email_to_blacklist(self, email, comment=""):
        ...
        """
        logger.info(
            "Function call: add_email_to_blacklist for '{}'".format(
                email,
            )
        )
        return (
            ...
            if not email
                ...
                self.__send_request("blacklist", "POST", {"emails": base64.b64encode(email), "comment": comment})
            )
        )
    ...
    def delete_email_from_blacklist(self, email):
        ...
        """
        logger.info(
            "Function call: delete_email_from_blacklist for '{}'".format(
                email,
            )
        )
        return (
            self.__handle_error("Empty email")
            if not email
            else self.__handle_result(self.__send_request("blacklist", "DELETE", {"emails": base64.b64encode(email)}))
        )
    ...
    def smtp_get_email_info_by_id(self, id):
        ...
        """
        logger.info(
            "Function call: smtp_get_email_info_by_id for '{}'".format(
                id,
            )
        )
        return (
    ...
    def smtp_add_emails_to_unsubscribe(self, emails):
        ...
        return (
            self.__handle_error("Empty email")
            if not emails
            else self.__handle_result(self.__send_request("smtp/unsubscribe", "POST", {"emails": json.dumps(emails)}))
        )
    ...
    def smtp_delete_emails_from_unsubscribe(self, emails):
        ...
        return (
            self.__handle_error("Empty email")
            if not emails
            else self.__handle_result(self.__send_request("smtp/unsubscribe", "DELETE", {"emails": json.dumps(emails)}))
        )
    ...
    def smtp_add_domain(self, email):
        ...
        return (
            self.__handle_error("Empty email")
            if not email
            else self.__handle_result(self.__send_request("smtp/domains", "POST", {"email": email}))
        )
    ...
    def smtp_send_mail(self, email):
        ...
        if not email.get("template") and not email.get("html") and not email.get("text"):
            ...
            return self.__handle_error(
                "Seems we have empty some credentials 'from': '{}' or 'to': '{}' fields".format(
    ...
    def push_get_tasks(self, limit=0, offset=0):
        ...
        return self.__handle_result(
            self.__send_request("push/tasks", "GET", {"limit": limit or 0, "offset": offset or 0})
        )
    ...
    def push_get_websites(self, limit=0, offset=0):
        ...
        return self.__handle_result(
            self.__send_request("push/websites", "GET", {"limit": limit or 0, "offset": offset or 0})
        )
    ...
    def push_get_subscriptions(self, id, limit=0, offset=0):
        ...
        return self.__handle_result(
            self.__send_request(
                "push/websites/{}/subscriptions".format(id), "GET", {"limit": limit or 0, "offset": offset or 0}
            )
        )
    ...
    def push_set_subscription_state(self, subscription_id, state_value):
        ...
        """
        logger.info(
            "Function call: push_set_subscription_state for {} to state {}".format(subscription_id, state_value)
        )
        return self.__handle_result(
            self.__send_request("/push/subscriptions/state", "POST", {"id": subscription_id, "state": state_value})
        )
    ...
    def push_create(self, title, website_id, body, ttl, additional_params={}):
        ...
        @return: dictionary with response message
        """
        data_to_send = {"title": title, "website_id": website_id, "body": body, "ttl": ttl}
    ...
    def sms_add_phones(self, addressbook_id, phones):
        ...
        data_to_send = {"addressBookId": addressbook_id, "phones": phones}
    ...
    def sms_get_phone_info(self, addressbook_id, phone):
        ...
        return self.__handle_result(
            self.__send_request("sms/numbers/info/" + str(addressbook_id) + "/" + str(phone), "GET")
        )
    ...
    def sms_update_phones_variables(self, addressbook_id, phones, variables):
        ...
        data_to_send = {"addressBookId": addressbook_id, "phones": phones, "variables": variables}
    ...
    def sms_add_phones_to_blacklist(self, phones, comment):
        ...
        data_to_send = {"phones": phones, "description": comment}
    ...
    def sms_add_campaign_by_addressbook_id(self, sender_name, addressbook_id, body, additional_params={}):
        ...
        data_to_send = {"sender": sender_name, "addressBookId": addressbook_id, "body": body}
    ...
    def sms_get_list_campaigns(self, date_from, date_to):
        ...
        data_to_send = {"dateFrom": date_from, "dateTo": date_to}
    ...
    def sms_get_campaign_info(self, id):
        ...
        logger.info(
            "Function call: sms_get_campaign_info from: {}".format(
                id,
            )
        )
        return self.__handle_result(
            self.__send_request(
                "/sms/campaigns/info/{}".format(
                    id,
                )
            )
        )
    ...
    def sms_cancel_campaign(self, id):
        ...
        logger.info(
            "Function call: sms_cancel_campaign : '{}'".format(
                id,
            )
        )
        return self.__handle_result(
            self.__send_request(
                "sms/campaigns/cancel/{}".format(
                    id,
                ),
                "PUT",
            )
    ...
    def sms_get_campaign_cost(self, sender, body, addressbook_id=None, phones=None):
        ...
        data_to_send = {"sender": sender, "body": body, "addressBookId": addressbook_id}
        if phones:
            try:
